...
class UserForm(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        if self.instance.pk:  # Verifica se é uma atualização de usuário existente
            existing_users = existing_users.exclude(pk=self.instance.pk)
        if existing_users.exists():
            raise ValidationError(_('Este nome de usuário já está em uso. Por favor, escolha outro.'))
        if not all(char.isalnum() or char in '@.+_- ' for char in username):
            raise ValidationError(_('O nome de usuário pode conter apenas letras, números e @/./+/-/_/ espaços.'))
        return username
