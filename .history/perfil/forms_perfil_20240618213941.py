...
class PerfilForm(forms.ModelForm):
    class Meta:
        model = Perfil
        fields = ()  # Deixe vazio se não houver campos adicionais a serem incluídos

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.user_creation_form = UserCreationForm()

    def clean(self):
        cleaned_data = super().clean()
        self.user_creation_form.cleaned_data = self.cleaned_data
        if not self.user_creation_form.is_valid():
            for field, errors in self.user_creation_form.errors.items():
                for error in errors:
    ...
    def save(self, commit=True):
        user = self.user_creation_form.save(commit=False)
        if commit:
            user.save()
