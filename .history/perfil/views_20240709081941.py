...
class CriarPerfilView(View):
    ...
    def post(self, request):
        ...
        if user_form.is_valid() and perfil_form.is_valid():
            ...
            perfil.save()
            login(request, user)
            next_url = request.POST.get('next') or  'produto:lista'
...
class ListaPedidosView(View):
    ...
    def get(self, request):
        if request.user.is_authenticated:
            pedidos_confirmados = Pedido.objects.filter(usuario=request.user, status='confirmado')
            return render(request, self.template_name, {'pedidos_confirmados': pedidos_confirmados}) # Redireciona para a página de login se não estiver autenticado
        else:
            return redirect('perfil:login') 