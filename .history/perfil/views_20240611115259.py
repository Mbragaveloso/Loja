...
class Criar(BasePerfil):
    def post(self, request, *args, **kwargs):
        ...
        if user_form.is_valid() and perfil_form.is_valid():
            # Salvar o usuário
            user = user_form.save()
            # Salvar o perfil com o usuário associado
            ...
            if password and not self.request.user.is_authenticated:
                autentica = authenticate(request, username=username, password=password)
                if autentica:
                    login(request, user=usuario)
                
            messages.success(request, 'Seu cadastro foi criado ou atualizado com sucesso.')
            messages.success(request, 'Você fez login e pode concluir sua compra.')
            return redirect('produto:carrinho')
        else:
            messages.error(request, 'Existem erros no formulário de cadastro. Verifique se todos os campos foram preenchidos corretamente.')
            return render(request, self.template_name, contexto)
