...
class Produto(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Verifica se o slug está duplicado e gera um novo se necessário
            contador = 1
            while Produto.objects.filter(slug=self.slug).exclude(pk=self.pk).exists():
                self.slug = f'{slugify(self.nome)}-{contador}'
                contador += 1
...
class Carrinho(models.Model):
    ...
    def get_total(self):
        ...
        for item in self.itemcarrinho_set.all():
            preco_unitario = item.produto.preco_marketing_promocional or item.produto.preco_marketing
...
class ItemPedido(models.Model):
    ...
    def __str__(self):
        return f'Item {self.id} do {self.pedido}'