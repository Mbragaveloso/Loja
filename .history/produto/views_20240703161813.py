...
class FinalizarCompra(View):
    def post(self, request, *args, **kwargs):
        ...
        for item in carrinho.values():
            preco_unitario = item.get('preco_unitario_promocional') if item.get('preco_unitario_promocional') else item.get('preco_unitario')
            total += preco_unitario * item['quantidade']
        
        # Aqui você pode adicionar a lógica adicional, como criar um pedido, atualizar estoques, etc.

        # Após finalizar, você pode redirecionar para uma página de confirmação
