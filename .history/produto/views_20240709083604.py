...
class ListaPedidos(LoginRequiredMixin, ListView):
    model = Pedido  # Defina o modelo correto aqui (assumindo que Pedido é o modelo para pedidos)
    template_name = 'perfil/meus_pedidos.html'  # Template para exibir a lista de pedidos
    context_object_name = 'pedidos'  # Nome do contexto para a lista de pedidos

    def get_queryset(self):
        # Filtra os pedidos confirmados para o usuário logado
        return Pedido.objects.filter(usuario=self.request.user, status='confirmado')
