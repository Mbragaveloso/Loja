...
class CarrinhoView(View):
    carrinho = request.session.get('carrinho', {})
    total = sum(item['quantidade'] * item.get('preco_unitario_promocional', item['preco_unitario']) for item in carrinho.values())
    context = {
        'carrinho': carrinho,
        'total': total,
    }
...
class ResumoDaCompra(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for item_id, detalhes_produto in carrinho.items():
            ...
            # Debugging
            print(f"Produto: {nome_produto}, Preço Unitário: {preco_unitario}, Quantidade: {quantidade}, Subtotal: {subtotal}, Total: {total}")
