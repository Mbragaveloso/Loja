...
class ResumoDaCompra(View):
    template_name = 'produto/resumo_da_compra.html'  # ajuste o nome do template conforme necessário
   def get(self, request, *args, **kwargs):
        # Exemplo: recuperar os itens do carrinho da sessão
        carrinho = request.session.get('carrinho', {})
        
        # Exemplo: recuperar detalhes dos produtos no carrinho
        produtos_no_carrinho = []
        total = 0
        context = {
            for produto_id, detalhes_produto in carrinho.items():
            # Lógica para recuperar detalhes do produto (exemplo: nome, preço, quantidade, etc.)
        ...
        return render(request, 'seu_template_de_resumo_de_compra.html', context)
